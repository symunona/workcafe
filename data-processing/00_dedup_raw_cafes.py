...
def main():
    ...
    parser.add_argument("--db", default=os.path.abspath(os.path.join(_HERE, '..', '..', 'data', 'seoul', 'scraped.db')), help="Path to DB")
    ...
    for provider, lat, lon, _ in rows:
        entries = conn.execute(
            "SELECT id, scraped_at FROM scraped_cafes WHERE provider=? AND lat=? AND lon=? ORDER BY scraped_at DESC, length(id) DESC",
            (provider, lat, lon)
        ).fetchall()
    ...
    remaining = conn.execute(
        "SELECT COUNT(*) FROM (SELECT provider, lat, lon FROM scraped_cafes GROUP BY provider, lat, lon HAVING COUNT(*) > 1)"
    ).fetchone()[0]
